...
class Sea_Attention(torch.nn.Module):
    ...
    def forward(self, x):  
        B, C, H, W = x.shape # [4,192,32,32]

        q = self.to_q(x) # [4,128,32,32]
        k = self.to_k(x) # [4,128,32,32]
        v = self.to_v(x) # [4,256,32,32]
        
        # detail enhance
        qkv = torch.cat([q, k, v], dim=1)
        qkv = self.act(self.dwconv(qkv))
        qkv = self.pwconv(qkv) # [4,192,32,32]

        # squeeze axial attention
        ## squeeze row
        qrow = self.pos_emb_rowq(q.mean(-1)).reshape(B, self.num_heads, -1, H).permute(0, 1, 3, 2) # [4,8,32,16]
        krow = self.pos_emb_rowk(k.mean(-1)).reshape(B, self.num_heads, -1, H) # [4,8,16,32]
        vrow = v.mean(-1).reshape(B, self.num_heads, -1, H).permute(0, 1, 3, 2) # [4,8,32,32]
        attn_row = torch.matmul(qrow, krow) * self.scale # [4,8,32,32]
        attn_row = attn_row.softmax(dim=-1)
        xx_row = torch.matmul(attn_row, vrow)  # B nH H C
        xx_row = self.proj_encode_row(xx_row.permute(0, 1, 3, 2).reshape(B, self.dh, H, 1)) # [4,256,32,1]
